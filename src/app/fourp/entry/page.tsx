...
export default function FourPEntryPage() {
  const { toast } = useToast();
  const [fourPLots, setFourPLots] = useLocalStorage<FourPLot[]>(FOURP_LOTS_KEY, []);
  ...
  const handleSaveLot = () => {
    ...
    setFourPLots([...fourPLots, newLot]);
    toast({ title: 'Success', description: '4P Lot created successfully.' });
    // Reset state
    setBarcode('');
    setLotDetails(null);
    setPackets([]);
  ...
  const handleReturnLot = () => {
    ...
    const totalAmount = pcs * priceMaster.fourP;

    const updatedLots = fourPLots.map(lot =>
  ...
  const unreturnedLots = fourPLots.filter(lot => !lot.isReturned);
  const returnedLots = fourPLots.filter(lot => lot.isReturned).sort((a,b) => new Date(b.returnDate!).getTime() - new Date(a.returnDate!).getTime());

  return (
    <div className="container mx-auto py-8 px-4 md:px-6 space-y-8">
      <PageHeader title="4P Entry & Return" description="Manage 4P lot creation and returns." />

      {!lotDetails ? (
        <Card>
          <CardHeader>
            <CardTitle>Step 1: Scan Lot Barcode</CardTitle>
            <CardDescription>Scan a barcode to begin creating a new 4P lot.</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <Card>
        <CardHeader><CardTitle>Unreturned 4P Lots</CardTitle></CardHeader>
        <CardContent>
